...
In vector calculus, vector fields are used to represent the flow of fluids, the movement of electrically charged particles, and even the direction of gravitational forces. Vector fields can be used to model everything from electric and magnetic fields to the flow of fluids. They're also pretty handy for navigation; if you know the vector field of a current, you can use it to figure out where you'll end up if you start floating downstream. So vector fields are pretty useful things to know about... even if they can be a bit confusing at first. But don't worry, with a little practice, you'll be calculating vector fields like a pro in no time!
...
::: {#def- } 
A \index{vector field} in $\mathbb{R}^n$ is a function $\mathbf{V} **vector field} in $\mathbb{R}^n$ is a function $\mathbf{V** $ that assigns a vector from each point in its domain.
A vector field  with domain $D$ in 
$\mathbb{R}^n$ has the form  
\[
\vec{V}(x_1,...,x_n)=\langle u_1(x_1,...,x_n),...,u_n(x_1,...,x_n) \rangle
\] 
where the scalar functions $u_1,$,...,$u_n$ are called the components of $\mathbf{V}.$ 
::: 
...
A vector field $\vec{V}$ is said to be \index{conservative} in a region $D$ if $\vec{V}=\nabla f$ for some scalar function $f$ in $D.$ The function $f$ is called a \index{scalar potential} of $\vec{V} **conservative} in a region $D$ if $\vec{V}=\nabla f$ for some scalar function $f$ in $D.$ The function $f$ is called a \index{scalar potential} of $\vec{V** $ in $D$.
::: 
...
Suppose that the vector field $\vec{V}$ and $\Curl  \vec{V}$ are both continuous in the simply connected region $D$ of $\mathbb{R}^3.$ Then $\vec{V}$ is conservative in $D$ if and only if $\Curl  \vec{V}=\vec{0}.$
::: 
